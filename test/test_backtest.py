...
class ComparisonTableGenerator:
    ...
    def generate_ranking_table(self, results_summary: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
        ...
        for strategy_name, metrics in results_summary.items():
            ...
            if max_return > min_return:
                ...
                return_score = 50  # If all strategies have same return, give medium score
            ...
            var_score = max(0, 100 - (var_95 * 100))  # VaR is usually negative, take absolute value
            ...
            sortino_score = min(100, max(0, sortino * 20 + 50))  # Simple linear transformation
            calmar_score = min(100, max(0, calmar * 20 + 50))
            
            scores['Risk-Adjusted Return'] = (sharpe_score + sortino_score + calmar_score) / 3
            ...
            if trade_count == 0:
                ...
                trade_score = 100 - (trade_count - 10) * 2  # Gradual deduction for 10-50 trades
            else:
                trade_score = max(0, 100 - trade_count)  # Large deduction for over 50 trades
            
            scores['Trading Efficiency'] = (win_rate_score + pl_score + trade_score) / 3
            
            # Calculate composite score
            weights = {'Return Performance': 0.3, 'Risk Control': 0.3, 'Risk-Adjusted Return': 0.25, 'Trading Efficiency': 0.15}
    ...
    def generate_statistical_significance_table(self, comparison_results: Optional[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for comparison_key, result in comparisons.items():
            try:
                ...
                self.logger.warning(f"Error processing comparison result {comparison_key}: {e}")
                continue
    ...
    def generate_ai_agent_analysis_table(self, results_summary: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
        ...
        analysis_data = [{
            ...
            'Difference': round(ai_metrics.get('total_return', 0) - avg_metrics['total_return'], 2),
            'Ranking': f"{rankings['total_return']}/{len(results_summary)}",
            'Performance': 'Above Average' if ai_metrics.get('total_return', 0) > avg_metrics['total_return'] else 'Below Average'
        }, {
            'Metric': 'Sharpe Ratio',
            'AI Agent': round(ai_metrics.get('sharpe_ratio', 0), 3),
            'Benchmark Average': round(avg_metrics['sharpe_ratio'], 3),
            'Difference': round(ai_metrics.get('sharpe_ratio', 0) - avg_metrics['sharpe_ratio'], 3),
            'Ranking': f"{rankings['sharpe_ratio']}/{len(results_summary)}",
            'Performance': 'Above Average' if ai_metrics.get('sharpe_ratio', 0) > avg_metrics['sharpe_ratio'] else 'Below Average'
        }, {
            'Metric': 'Max Drawdown(%)',
            'AI Agent': round(abs(ai_metrics.get('max_drawdown', 0)), 2),
            'Benchmark Average': round(abs(avg_metrics['max_drawdown']), 2),
            'Difference': round(abs(ai_metrics.get('max_drawdown', 0)) - abs(avg_metrics['max_drawdown']), 2),
            'Ranking': f"{rankings['max_drawdown']}/{len(results_summary)}",
            'Performance': 'Above Average' if abs(ai_metrics.get('max_drawdown', 0)) < abs(avg_metrics['max_drawdown']) else 'Below Average'
        }, {
            'Metric': 'Annual Volatility(%)',
            'AI Agent': round(ai_metrics.get('volatility', 0), 2),
            'Benchmark Average': round(avg_metrics['volatility'], 2),
            'Difference': round(ai_metrics.get('volatility', 0) - avg_metrics['volatility'], 2),
            'Ranking': f"{rankings['volatility']}/{len(results_summary)}",
            'Performance': 'Above Average' if ai_metrics.get('volatility', 0) < avg_metrics['volatility'] else 'Below Average'
        }]
...
class BacktestTest:
    ...
    def select_strategies_by_duration(self) -> list:
        ...
        if self.test_duration_days >= 30:
            ...
            self.logger.info("✓ Added short-term moving average strategy (requires 20 days)")
        ...
        if self.test_duration_days >= 60:
            ...
            self.logger.info("✓ Added standard moving average strategy (requires 30 days)")
    ...
    def test_baseline_strategies_backtest(self):
        ...
        # Validate results
        assert len(results) > 0, "At least one strategy should complete backtest successfully"
        ...
        for name, result in results.items():
            ...
            self.logger.info(f"  - {name}: Return {total_return:.2f}%, Sharpe {sharpe_ratio:.3f}")
    ...
    def test_ai_agent_backtest(self, ticker: str = "000001"):
        ...
        try:
            ...
            self.logger.info(f"  Total return: {agent_result.performance_metrics.get('total_return', 0)*100:.2f}%")
            self.logger.info(f"  Sharpe ratio: {agent_result.performance_metrics.get('sharpe_ratio', 0):.3f}")
    ...
    def test_comprehensive_comparison(self, ticker: str = "000001"):
        ...
        self.logger.info("Testing comprehensive comparison between AI Agent and baseline strategies...")
        ...
        try:
            ...
            if self.test_duration_days >= 30:  # AI agent needs at least 30 days of data
                ...
                if agent_result:
                    ...
                    # Debug output
                    if abs(ai_original_total_return) < 1e-6:  # If total return is very small
                        self.logger.warning(f"AI Agent total return is very small: {ai_original_total_return}, daily returns array length: {len(agent_result.daily_returns) if hasattr(agent_result, 'daily_returns') else 0}")
                        if hasattr(agent_result, 'daily_returns') and len(agent_result.daily_returns) > 0:
                            self.logger.warning(f"  AI Agent daily returns sample: {agent_result.daily_returns[:5].tolist() if len(agent_result.daily_returns) >= 5 else agent_result.daily_returns.tolist()}")
                ...
                self.logger.warning("Test duration too short, skipping AI Agent backtest")
        ...
        for name, result in baseline_results.items():
            ...
            if abs(original_total_return) < 1e-6:  # If total return is very small
                self.logger.warning(f"Strategy {name} total return is very small: {original_total_return}, daily returns array length: {len(result.daily_returns) if hasattr(result, 'daily_returns') else 0}")
                if hasattr(result, 'daily_returns') and len(result.daily_returns) > 0:
                    self.logger.warning(f"  Daily returns sample: {result.daily_returns[:5].tolist() if len(result.daily_returns) >= 5 else result.daily_returns.tolist()}")
            
            # Handle small return value display - preserve more precision
            total_return_pct = round(original_total_return * 100, 4)  # Keep 4 decimal places
        ...
        if len(backtester.results) >= 2:
            try:
                ...
                if not significance_df.empty:
                    ...
                    self.logger.info(f"Statistical significance table saved to: {significance_filename}")
    ...
    def _generate_comparison_report(self, results_summary: dict):
        ...
        self.logger.info(f"{'Rank':<4} {'Strategy Name':<25} {'Return':<12} {'Sharpe Ratio':<12} {'Max Drawdown':<12}")
        self.logger.info("-" * 80)
        ...
        if 'AI_Agent' in results_summary:
            ...
            self.logger.info(f"  📊 Ranking: #{ai_rank} (out of {len(sorted_results)} strategies)")
            self.logger.info(f"  💰 Return: {ai_metrics['total_return']:.2f}%")
            self.logger.info(f"  📈 Sharpe Ratio: {ai_metrics['sharpe_ratio']:.3f}")
            self.logger.info(f"  📉 Max Drawdown: {abs(ai_metrics['max_drawdown']):.2f}%")
            ...
            self.logger.info(f"  Return difference: {ai_metrics['total_return'] - avg_return:+.2f}%")
            self.logger.info(f"  Sharpe ratio difference: {ai_metrics['sharpe_ratio'] - avg_sharpe:+.3f}")
            self.logger.info(f"  Drawdown difference: {abs(ai_metrics['max_drawdown']) - avg_drawdown:+.2f}%")
    ...
    def _generate_final_test_report(self, test_results: dict):
        ...
        self.logger.info(f"  Time Range: {config['start_date']} to {config['end_date']}")
        self.logger.info(f"  Test Duration: {config['duration_days']} days")
        self.logger.info(f"  Test Stock: {config['ticker']}")
        
        # Test completion status
        self.logger.info(f"\nTest Completion Status:")
        self.logger.info(f"  Component Initialization: {'✓' if test_results.get('initialization') else '✗'}")
        self.logger.info(f"  Baseline Strategies: {len(test_results.get('baseline_results', {}))} strategies")
        self.logger.info(f"  AI Agent Test: {'✓' if test_results.get('ai_agent_result') else '✗'}")
        self.logger.info(f"  Comparison Analysis: {'✓' if test_results.get('comparison_results') else '✗'}")
...
def main():
    ...
    parser.add_argument('--quick', action='store_true', help='Quick test mode (short time)')
    parser.add_argument('--medium', action='store_true', help='Medium test mode (medium time)')
    parser.add_argument('--full', action='store_true', help='Full test mode (long time)')
    ...
    parser.add_argument('--baseline-only', action='store_true', help='Test baseline strategies only')
    parser.add_argument('--comparison', action='store_true', help='Run comprehensive comparison test')
