...
def analyze_sentiment_with_llm(text):
    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a professional financial sentiment analysis AI. Please analyze the sentiment tendency of the following financial news and score its impact on the stock market/enterprise. Score range from -1 (extremely negative) to 1 (extremely positive), only return a number, no explanation."},
                {"role": "user", "content": f"Perform sentiment analysis scoring (-1 to 1) on the following financial news, only return a number:\n\n{text}"}
            ],
            "temperature": 0.3
        }
        ...
        if response.status_code == 200:
            ...
            st.warning(f"API request failed, status code: {response.status_code}, using neutral score")
            sentiment_score = 0.0
...
def extract_news_insights_with_llm(news_df):
    ...
    try:
        ...
        payload = {
            "model": LLM_API_CONFIG["model_name"],
            "messages": [
                {"role": "system", "content": "You are a professional financial analyst. Please analyze the following financial news titles and extract key market trends and insights."},
                {"role": "user", "content": f"Please analyze the following financial news titles, identify main market trends, potential investment opportunities and risks. Provide concise analysis (within 200 words):\n\n{titles_text}"}
            ],
            "temperature": 0.7
        }
...
def interpret_prediction_with_llm(stock_name, prediction_data, model_name, metrics):
    ...
    try:
        ...
        payload = {
            "model": LLM_API_CONFIG["model_name"],
            "messages": [
                {"role": "system", "content": "You are a professional financial analyst, skilled at interpreting stock prediction model results."},
                {"role": "user", "content": f"Please interpret the following {stock_name} stock prediction results, provide professional analysis insights and possible investment advice (around 200 words):\n\n{prediction_text}"}
            ],
            "temperature": 0.7
        }
...
def get_stock_list():
    ...
    try:
        ...
        while (rs.next()):
            ...
            if row[4] == '1' and row[5] == '1':  # type=1 means A-share, status=1 means listed
...
def fetch_index_data(index_code, start_date, end_date):
    ...
    try:
        ...
        if not data_list:
            st.warning(f"No data found for index {index_code} from {start_date} to {end_date}")
            return None
            ...
            for item in news_items:
                try:
                    ...
                    if time_elem:
                        ...
                        if '年' in date_str and '月' in date_str and '日' in date_str:  # Check for Chinese date format (year, month, day)
                            date_match = re.search(r'(\d{4})年(\d{1,2})月(\d{1,2})日', date_str)  # Parse Chinese date format: YYYY年MM月DD日
                            if date_match:
...
def plot_news_sentiment(news_df):
    ...
    if 'date' in news_df.columns:
        ...
        ax.set_ylabel('Sentiment Score (Negative→Positive)', fontproperties='SimHei', fontsize=12)
        ax.grid(True, axis='y')
...
def plot_news_sentiment_distribution(news_df):
    ...
    ax.set_title('News Sentiment Distribution Histogram', fontproperties='SimHei', fontsize=14)
...
def data_acquisition_tab():
        ...
        if stock_input_type == "Direct input":
            ...
            try:
                ...
                if stock_list:
                    ...
                    stock_options = [f"{stock['code'].split('.')[-1]} - {stock['name']}" for stock in stock_list[:1000]]  # Limit quantity to avoid too many
        ...
        if st.button("Get stock data"):
            with st.spinner("Getting stock data..."):
                ...
                if stock_df is not None and not stock_df.empty:
                    ...
                    st.success(f"Successfully obtained {symbol} stock data, {len(stock_df)} records")
                ...
                with st.spinner("Getting stock basic info..."):
                    try:
                        ...
                        if stock_info:
                            ...
                            st.write(f"Delisting date: {stock_info[3] if stock_info[3] else 'Present'}")
                            st.write(f"Stock type: {'A-share' if stock_info[4]=='1' else 'Other'}")
                            st.write(f"Status: {'Listed' if stock_info[5]=='1' else 'Delisted'}")
                        else:
    ...
    with col2:
        ...
        selected_index = st.selectbox("Select index", list(major_indices.items()), format_func=lambda x: x[1])
        
        index_start_date = st.date_input("Index start date", datetime.now() - timedelta(days=365), key="index_start")
        index_end_date = st.date_input("Index end date", datetime.now(), key="index_end")
        ...
        if st.button("Get index data"):
            with st.spinner("Getting index data..."):
                ...
                if index_df is not None and not index_df.empty:
                    ...
                    st.success(f"Successfully obtained {selected_index[1]} index data, {len(index_df)} records")
    ...
    if st.button("Get industry classification data"):
        with st.spinner("Getting industry classification data..."):
            ...
            if industry_df is not None and not industry_df.empty:
                ...
                st.success(f"Successfully obtained industry classification data, {len(industry_df)} records")
                ...
                fig, ax = plt.subplots(figsize=(10, 6))
                ax.bar(industry_counts['Industry'][:15], industry_counts['Number of listed companies'][:15], 
                       color='steelblue', alpha=0.7)
                ax.set_xlabel('Industry', fontproperties='SimHei', fontsize=12)
                ax.set_ylabel('Number of listed companies', fontproperties='SimHei', fontsize=12)
                ax.set_title('Number of listed companies by industry (Top 15)', fontproperties='SimHei', fontsize=14)
    ...
    with calendar_col1:
        calendar_start = st.date_input("Calendar start date", datetime.now() - timedelta(days=30), key="calendar_start")
    with calendar_col2:
        calendar_end = st.date_input("Calendar end date", datetime.now(), key="calendar_end")
    ...
    if st.button("Get trading calendar"):
        with st.spinner("Getting trading calendar..."):
            ...
            if calendar_df is not None and not calendar_df.empty:
                ...
                st.success(f"Successfully obtained trading calendar, {len(calendar_df)} records")
    ...
    if uploaded_file is not None:
        try:
            ...
            if all(col in df.columns for col in ['open', 'high', 'low', 'close']):
                st.info("Detected uploaded stock data, can be processed in data analysis tab")
        except Exception as e:
...
def data_analysis_tab():
    ...
    if 'stock_df' in st.session_state:
        ...
        if 'processed_df' in st.session_state:
            ...
            if viz_type == "Candlestick Chart":
                ...
                if st.button("Generate Return Distribution Chart"):
                    ...
                    ax.set_title("Daily Return Distribution", fontproperties='SimHei', fontsize=14)
        ...
        if all(col in st.session_state.uploaded_df.columns for col in ['open', 'high', 'low', 'close']):
            ...
            st.info("Uploaded file is not in standard stock data format, please ensure it contains open, high, low, close columns")
    else:
    ...
    if 'index_df' in st.session_state:
        ...
        if st.button("Analyze Index Data"):
            with st.spinner("Analyzing index data..."):
                ...
                ax.set_title(f'{index_name} Daily Return Distribution', fontproperties='SimHei', fontsize=14)
                ax.set_xlabel('Daily Return', fontproperties='SimHei', fontsize=12)
                ax.set_ylabel('Frequency', fontproperties='SimHei', fontsize=12)
                ax.grid(True, alpha=0.3)
                plt.tight_layout()
                ...
                if 'processed_df' in st.session_state:
                    ...
                    if common_dates:
                        ...
                        st.write(f"Stock and {index_name} return correlation coefficient: {correlation:.4f}")
                        
                        # Plot scatter chart
                        fig, ax = plt.subplots(figsize=(10, 6))
                        ax.scatter(index_returns, stock_returns, alpha=0.5)
                        ax.set_title(f'Stock Return vs {index_name} Return (Correlation: {correlation:.4f})', 
                                   fontproperties='SimHei', fontsize=14)
                        ax.set_xlabel(f'{index_name} Daily Return', fontproperties='SimHei', fontsize=12)
                        ax.set_ylabel('Stock Daily Return', fontproperties='SimHei', fontsize=12)
                        ax.grid(True, alpha=0.3)
                        ...
                        st.write(f"Beta Explanation: {'Higher than market volatility' if beta > 1 else 'Lower than market volatility'}")
    ...
    if 'news_df' in st.session_state:
        ...
        if 'sentiment' in st.session_state.news_df.columns:
            ...
            if fig:
                ...
                st.info("Unable to generate sentiment analysis chart, please ensure news data contains sentiment scores and dates")
    ...
    if 'industry_df' in st.session_state:
        ...
        if st.button("Analyze industry stocks"):
            with st.spinner("Analyzing industry stocks..."):
                ...
                st.write(f"{selected_industry} industry listed companies: {len(industry_stocks)} companies")
                ...
                if 'processed_df' in st.session_state and 'index_df' in st.session_state:
                    ...
                    # Prompt user
                    st.write(f"Does the stock you are currently analyzing ({stock_symbol}) belong to the {selected_industry} industry:")
                    ...
                    st.write("For complete industry index data, please use professional data sources")
    ...
    if 'processed_df' in st.session_state:
        ...
        with col1:
            ...
            if st.button("Train regression model"):
                with st.spinner("Training model..."):
                    ...
                    # Display results
                    st.success(f"Model training completed, Mean Squared Error (MSE): {mse:.6f}, Root Mean Squared Error (RMSE): {rmse:.6f}")
        ...
        if st.button("Execute clustering analysis"):
            with st.spinner("Executing clustering analysis..."):
                ...
                st.success(f"Clustering analysis completed, {n_clusters} clusters in total")
...
def visualization_tab():
    ...
    if 'processed_df' in st.session_state:
        ...
        viz_option = st.selectbox(
            "Select visualization type",
            ['Candlestick Chart', 'Price Trend Chart', 'Technical Indicators', 'Correlation Heatmap']
        )
    ...
    if 'regression_results' in st.session_state or 'classification_results' in st.session_state:
        ...
        if pred_viz_option == 'Regression Prediction' and 'regression_results' in st.session_state:
        ...
        elif pred_viz_option == 'Classification Prediction' and 'classification_results' in st.session_state:
    ...
    if 'cluster_df' in st.session_state:
        ...
        col1, col2 = st.columns(2)
        with col1:
            x_feature = st.selectbox("X-axis feature", ['Daily_Return', 'Volatility_5d', 'RSI', 'close'])
        with col2:
            y_feature = st.selectbox("Y-axis feature", ['Volatility_5d', 'Daily_Return', 'RSI', 'close'])
    ...
    if 'news_df' in st.session_state:
        ...
        news_viz_option = st.radio(
            ...
            ['Sentiment Analysis', 'Sentiment Distribution Histogram', 'Keyword Word Cloud'],
            horizontal=True
        )
        ...
        if news_viz_option == 'Sentiment Analysis':
            ...
            if fig:
                ...
                st.info("Unable to generate sentiment distribution chart, please ensure news data contains sentiment scores")
            ...
            if fig:
                ...
                st.info("Unable to generate word cloud, please ensure news data contains keywords or titles")
...
def export_tab():
    ...
    export_option = st.selectbox(
        "Select data to export",
        ['Raw Stock Data', 'Processed Stock Data', 'Prediction Results', 'News Data', 'Index Data']
    )
    ...
    if st.button("Export data"):
        ...
        elif export_option == 'Processed Stock Data' and 'processed_df' in st.session_state:
            if export_format == 'CSV':
                st.markdown(get_table_download_link(
        ...
        try:
            ...
            if result:
                predictions_df = pd.DataFrame(result, columns=[
                    'Stock Code', 'Model Name', 'Prediction Date', 'Predicted Value', 'Actual Value', 'Accuracy'
                ])
...
def main():
    ...
    # Main interface tabs
    tabs = st.tabs(["Data Acquisition", "Data Analysis", "Machine Learning Prediction", "Visualization", "Export Results"])
