...
class ModelEvaluator:
    ...
    def split_data(self, data: pd.DataFrame, train_ratio: float = 0.7, 
        ...
        logger.info(f"Data split - Training set: {len(train_data)} rows ({train_ratio*100:.1f}%), "
    ...
    def evaluate_regression_model(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        logger.info(f"{model_name} regression evaluation results on {dataset_name} set:")
    ...
    def evaluate_classification_model(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        logger.info(f"{model_name} classification evaluation results on {dataset_name} set:")
    ...
    def visualize_predictions(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        if date_index is not None and len(date_index) == len(y_true):
            plt.plot(date_index, y_true, label='True Values', marker='o', linestyle='-', markersize=3)
            plt.plot(date_index, y_pred, label='Predicted Values', marker='x', linestyle='--', markersize=3)
            plt.gcf().autofmt_xdate()
        else:
            plt.plot(y_true, label='True Values', marker='o', linestyle='-', markersize=3)
            plt.plot(y_pred, label='Predicted Values', marker='x', linestyle='--', markersize=3)
    ...
    def visualize_forecast(self, history: np.ndarray, forecast: np.ndarray, 
        ...
        if date_index is not None and len(date_index) == len(history) + len(forecast):
            ...
            plt.plot(x_forecast, forecast, label='Forecast', color='red', linestyle='--')
