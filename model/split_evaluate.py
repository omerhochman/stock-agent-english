...
def split_and_evaluate(
    ...
    if verbose:
        logger.info(
            f"Starting data splitting and model evaluation - Stock code: {ticker}"
        )
        logger.info(
            f"Data split ratio - Training set: {train_ratio*100:.1f}%, Validation set: {val_ratio*100:.1f}%, Test set: {test_ratio*100:.1f}%"
        )
    ...
    if verbose:
        logger.info(
            f"Data splitting completed - Training set: {train_data.shape}, Validation set: {val_data.shape}, Test set: {test_data.shape}"
        )
        ...
        evaluator.compare_train_test_distributions(
            ...
            test_data,
            columns=(
                ["close", "returns", "volatility_20d"]
                if all(
                    col in processed_data.columns
                    for col in ["close", "returns", "volatility_20d"]
                )
                else None
            ),
...
def train_and_evaluate_dl(
    ...
    if missing_features:
        if verbose:
            logger.warning(
                f"The following features are missing from the data: {missing_features}"
            )
        ...
        try:
            ...
            for i in range(seq_length, len(test_data)):
                ...
                pred = ml_agent.dl_module.predict_lstm(
                    ...
                    0
                ]  # Only take the first prediction value, i.e., next step prediction
    ...
    if ml_agent.dl_module.rf_model is not None:
        try:
            ...
            test_labels = ml_agent.dl_module.generate_training_labels(
                test_data, forward_days=model_params["forecast_days"]
            )
            ...
            if len(common_idx) > 0:
                # Get predictions
                test_features_scaled = ml_agent.dl_module.feature_scaler.transform(
                    test_features.loc[common_idx]
                )
                ...
                rf_metrics = evaluator.evaluate_classification_model(
                    test_true, test_pred, model_name=f"{ticker}_rf", dataset_name="test"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Error visualizing feature importance: {str(feat_error)}"
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Stock model data splitting and evaluation tool"
    )
