...
class FactorMiningModule:
        ...
        for func_name, param_values in ts_functions.items():
            ...
            for param in param_values:
                func = partial(
                    ...
                    **(
                        {"window": param}
                        if "roll" in func_name or "rank" in func_name
                        else {"period": param}
                    ),
                )
    ...
    def _evaluate_factor(
        ...
        try:
            ...
            factor_values = np.nan_to_num(
                factor_values, nan=0.0, posinf=0.0, neginf=0.0
            )

            # Check if factor values are valid
            if np.all(factor_values == 0.0) or np.all(
                np.isclose(factor_values, factor_values[0])
            ):
    ...
    def _calculate_ic(
        self, factor_values: np.ndarray, future_returns: np.ndarray, window: int = 20
    ) -> float:
    ...
    def _calculate_return_spread(
        self, factor_values: np.ndarray, future_returns: np.ndarray, quantiles: int = 5
    ) -> float:
        ...
        df["quantile"] = pd.qcut(
            df["factor"], q=quantiles, labels=False, duplicates="drop"
        )
    ...
    def _factor_fitness(
        self, factor_values: np.ndarray, future_returns: np.ndarray
    ) -> float:
        ...
        try:
            ...
            try:
                return_spread = self._calculate_return_spread(
                    factor_values_clean, future_returns_clean
                )
    ...
    def _calculate_monotonicity(
        self, factor_values: np.ndarray, future_returns: np.ndarray, n_groups: int = 5
    ) -> float:
        ...
        try:
            ...
            try:
                df["quantile"] = pd.qcut(
                    df["factor"], q=n_groups, labels=False, duplicates="drop"
                )
            except:
            ...
            try:
                ...
                trend = (group_returns.iloc[-1] - group_returns.iloc[0]) / (
                    len(group_returns) - 1
                )
    ...
    def evolve_factors(
        ...
        self.toolbox.register(
            "evaluate", self._evaluate_factor, X=X, y=y, eval_func=self._factor_fitness
        )
        ...
        try:
            self.logger.info(
                f"Starting factor evolution, population size: {population_size}, iterations: {n_generations}"
            )
            ...
            for i, ind in enumerate(hof):
                if ind.fitness.values[0] > min_fitness:
                    ...
                    self.logger.info(
                        f"Factor {factor_name} generation completed, fitness: {factor_info['fitness']:.4f}"
                    )
    ...
    def _prepare_data(
        self, price_data: pd.DataFrame, future_return_periods: int = 5
        ...
        feature_generators = {
            # Price features
            "open_close": lambda d: (
                d["open"] / d["close"] - 1 if "open" in d.columns else 0
            ),
            "high_low": lambda d: (
                d["high"] / d["low"] - 1
        ...
        df["future_return"] = (
            df["close"].pct_change(future_return_periods).shift(-future_return_periods)
        )
    ...
    def calculate_factor_values(
        ...
        try:
            ...
            if "open" in enhanced_df.columns and "close" in enhanced_df.columns:
                ...
                if any(valid_close):
                    enhanced_df.loc[valid_close, "open_close"] = (
                        enhanced_df.loc[valid_close, "open"]
                        / enhanced_df.loc[valid_close, "close"]
                        - 1
                    )
            ...
            if "high" in enhanced_df.columns and "low" in enhanced_df.columns:
                ...
                if any(valid_low):
                    enhanced_df.loc[valid_low, "high_low"] = (
                        enhanced_df.loc[valid_low, "high"]
                        / enhanced_df.loc[valid_low, "low"]
                        - 1
                    )
            ...
            for factor_info in self.best_factors:
                try:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error compiling factor {factor_name}: {eval_error}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error calculating factor {factor_info['name']} values: {str(e)}"
                    )
            ...
            self.logger.error(
                f"Error occurred during factor value calculation: {str(e)}"
            )
            return {}
    ...
    def _ensure_all_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "close" in enhanced_df.columns:
            ...
            for window in [5, 10, 20, 60]:
                ...
                if col_name not in enhanced_df.columns:
                    enhanced_df[col_name] = (
                        enhanced_df["close"].rolling(window=window).mean()
                    )
            ...
            for window in [5, 10, 20]:
                ...
                if col_name not in enhanced_df.columns:
                    enhanced_df[col_name] = (
                        enhanced_df["close"].pct_change().rolling(window=window).std()
                    )
            ...
            for short_win, long_win in ma_pairs:
                ...
                if cross_name not in enhanced_df.columns:
                    enhanced_df[cross_name] = (
                        enhanced_df[short_ma] / enhanced_df[long_ma] - 1
                    )
            ...
            if "macd" not in enhanced_df.columns:
                ...
                enhanced_df["macd_signal"] = (
                    enhanced_df["macd"].ewm(span=9, adjust=False).mean()
                )
                enhanced_df["macd_hist"] = (
                    enhanced_df["macd"] - enhanced_df["macd_signal"]
                )
    ...
    def _extract_variable_names(self, expr_str: str) -> List[str]:
        ...
        variables = [
            ...
            if var not in exclude_list and not var.startswith("ARG")
        ]
...
class FactorAgent:
    ...
    def generate_factors(self, price_data, n_factors=5, **kwargs):
        ...
        try:
            ...
            evolve_params = {"price_data": price_data, "n_factors": n_factors}

            # Add other possible parameters
            allowed_params = [
                "population_size",
                "n_generations",
                "future_return_periods",
                "min_fitness",
            ]
    ...
    def generate_signals(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for factor_name, factor_value in latest_values.items():
                ...
                factor_trend = (
                    factor_value - np.mean(factor_series[-6:-1])
                ...
                factor_std = (
                    np.std(factor_series[-20:]) if len(factor_series) > 20 else 1
                )
            ...
            avg_confidence = sum(s["confidence"] for s in factor_signals) / len(
                factor_signals
            )
            ...
            signals = {
                ...
                "reasoning": self._generate_reasoning(
                    factor_signals, final_signal, final_confidence
                ),
            }
    ...
    def _generate_reasoning(
        ...
        reasoning_parts.append(
            f"Genetic programming factor signals: {len(factor_signals)} factors, {bullish_count} bullish, {bearish_count} bearish, {neutral_count} neutral"
        )
        ...
        if strongest_bullish:
            ...
            reasoning_parts.append(
                f"Strongest bullish factor: {top_bullish['name']}, factor value: {top_bullish['value']:.2f}, confidence: {top_bullish['confidence']:.2f}"
            )

        if strongest_bearish:
            top_bearish = max(strongest_bearish, key=lambda s: s["confidence"])
            reasoning_parts.append(
                f"Strongest bearish factor: {top_bearish['name']}, factor value: {top_bearish['value']:.2f}, confidence: {top_bearish['confidence']:.2f}"
            )

        # Add final conclusion
        reasoning_parts.append(
            f"Comprehensive genetic programming factor analysis yields {final_signal} signal, confidence: {final_confidence:.2f}"
        )
