...
class DeepLearningModule:
    ...
    def __init__(self, model_dir: str = 'models'):
        ...
        logger.info(f"Deep learning module initialization completed, using device: {self.device}")
    ...
    def _create_lstm_sequences(self, data: np.ndarray, seq_length: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(len(data) - seq_length):
            ...
            y.append(data[i + seq_length, 0])  # Only predict first column (usually closing price)
    ...
    def train_lstm_model(self, 
        ...
        try:
            ...
            if missing_features:
                raise ValueError(f"The following feature columns do not exist in data: {missing_features}")
            ...
            logger.info(f"Initializing LSTM model, input dimension: {input_dim}, hidden dimension: {hidden_dim}, layers: {num_layers}")
    ...
    def predict_lstm(self, price_data: pd.DataFrame, feature_cols: Optional[List[str]] = None,
        ...
        if self.lstm_model is None:
            raise ValueError("LSTM model not trained, please call train_lstm_model method first")
        ...
        try:
            ...
            missing_features = [col for col in feature_cols if col not in price_data.columns]
            if missing_features:
                logger.warning(f"The following features are missing from data: {missing_features}")
        ...
        try:
            ...
            if len(class_distribution) > 1:
                ...
                if ratio_0 > 0.65 or ratio_1 > 0.65:
                    logger.info("Detected severe class imbalance, will apply balancing weights")
            ...
            logger.info(f"Validation set prediction class distribution: {val_pred_distribution.to_dict()}")
            
            logger.info(f"Classifier training completed, training accuracy: {train_accuracy:.4f}, validation accuracy: {val_accuracy:.4f}")
            ...
            logger.info(f"Feature importance: {sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)[:5]}")
    ...
    def predict_stock_returns(self, features: pd.DataFrame) -> Dict[str, float]:
        ...
        if self.rf_model is None:
            raise ValueError("Random forest model not trained, please call train_stock_classifier method first")
        ...
        try:
            ...
            logger.info(f"Original prediction probabilities - Up: {raw_probability_up:.4f}, Down: {raw_probability_down:.4f}")
            logger.info(f"Calibrated probabilities - Up: {calibrated_prob_up:.4f}, Down: {calibrated_prob_down:.4f}")
            ...
            return {
                ...
                "expected_return": (calibrated_prob_up - 0.5) * 2  # Convert to -1 to 1 range
            }
    ...
    def load_models(self, lstm_path: Optional[str] = None, rf_path: Optional[str] = None,
        ...
        try:
            ...
            if os.path.exists(lstm_path) and os.path.exists(price_scaler_path):
                ...
                logger.info(f"Feature dimension detected from price_scaler: {input_dim}")
                ...
                try:
                    ...
                    logger.info(f"Starting to try loading model parameters, detected the following parameters: {list(state_dict.keys())}")
                    ...
                    if 'lstm.weight_ih_l0' in state_dict:
                        ...
                        logger.info(f"Inferred from model parameters: hidden_size={actual_hidden_size}, input_dim={actual_input_dim}")
                        
                        # Check output layer size
                        if 'fc.weight' in state_dict:
                            fc_param_shape = state_dict['fc.weight'].shape
                            actual_output_dim = fc_param_shape[0]
                            logger.info(f"Inferred from model parameters: output_dim={actual_output_dim}")
                        else:
                        ...
                        while f'lstm.weight_ih_l{actual_num_layers}' in state_dict:
                            actual_num_layers += 1
                        logger.info(f"Inferred from model parameters: LSTM layers={actual_num_layers}")
                        
                        # Recreate model with correct parameters
                        logger.info(f"Recreating model with inferred parameters: input_dim={actual_input_dim}, hidden_dim={actual_hidden_size}, num_layers={actual_num_layers}, output_dim={actual_output_dim}")
                        ...
                        logger.info("✓ LSTM model loaded successfully, using model structure inferred from parameters")
                    else:
                        # If key parameters cannot be found, retry with default values
                        logger.warning("Cannot infer model structure from parameters, using default parameters")
                        self.lstm_model = LSTMModel(input_dim=input_dim, hidden_dim=128, num_layers=3, output_dim=10)
                        self.lstm_model.load_state_dict(state_dict)
                        logger.info("Model loaded successfully with default parameters")
                except Exception as e:
                    logger.error(f"Final model loading failure: {str(e)}")
                    logger.warning("Will use default model initialization, but will not load pretrained weights")
                    self.lstm_model = LSTMModel(input_dim=input_dim, hidden_dim=128, num_layers=3, output_dim=10)
...
class MLAgent:
    ...
    def train_models(self, price_data: pd.DataFrame, technical_indicators: Dict[str, pd.Series] = None,
        ...
        try:
            ...
            if feature_cols is None:
                feature_cols = ['close', 'ma5', 'ma10', 'ma20', 'rsi', 'macd']
                self.logger.info(f"No feature columns specified, using default features: {feature_cols}")
            
            # Ensure feature columns exist in data, otherwise add them
            missing_features = [col for col in feature_cols if col not in price_data.columns]
            if missing_features:
                self.logger.warning(f"Data missing the following features, will add automatically: {missing_features}")
                for col in missing_features:
                    if col == 'ma5':
                        price_data['ma5'] = price_data['close'].rolling(window=5).mean()
                    elif col == 'ma10':
            ...
            self.logger.info(f"Starting model training with parameters: seq_length={seq_length}, hidden_dim={hidden_dim}, "
            ...
            if len(common_idx) > 0:
                self.logger.info(f"Training random forest classifier with {len(common_idx)} samples")
                ...
                self.logger.warning("Features and labels have no common index, cannot train classifier")
    ...
    def generate_signals(self, price_data: pd.DataFrame, 
        ...
        try:
            ...
            if self.dl_module.lstm_model is not None:
                ...
                self.logger.info(f"Using LSTM model to predict future {forecast_days} days prices")
                ...
                self.logger.info(f"LSTM prediction will use the following features: {lstm_feature_cols}")
                ...
                self.logger.info(f"LSTM prediction - Average return: {avg_return:.4f}, Median return: {median_return:.4f}")
                self.logger.info(f"LSTM prediction - Positive return days: {positive_count}, Negative return days: {negative_count}")
                ...
                if avg_return > 0.02 and positive_ratio > 0.6:  # Positive signal threshold
                    signals['lstm_signal'] = 'bullish'
                    signals['lstm_confidence'] = min(0.5 + positive_ratio * 0.4, 0.9)  # More moderate confidence
                elif avg_return < -0.02 and positive_ratio < 0.4:  # Negative signal threshold
            ...
            if self.dl_module.rf_model is not None:
                ...
                if not features.empty:
                    ...
                    self.logger.info(f"RF prediction - Signal: {signals['rf_signal']}, Confidence: {signals['rf_confidence']:.4f}")
            ...
            if 'lstm_signal' in signals and 'rf_signal' in signals:
                ...
                self.logger.info(f"Final signal - {signals['signal']}, Confidence: {signals['confidence']:.4f}, Combined score: {combined_score:.4f}")
    ...
    def _generate_reasoning(self, signals: Dict[str, Any]) -> str:
        ...
        if 'rf_prediction' in signals:
            ...
            if pred == 1:
                reasoning_parts.append(f"Random forest model predicts up probability: {up_prob:.2%}, down probability: {down_prob:.2%}")
            else:
                reasoning_parts.append(f"Random forest model predicts down probability: {down_prob:.2%}, up probability: {up_prob:.2%}")
        
        # Technical indicator evaluation
        if 'lstm_signal' in signals:
            reasoning_parts.append(f"LSTM technical analysis result: {signals['lstm_signal']}, confidence: {signals['lstm_confidence']:.2%}")
        
        if 'rf_signal' in signals:
            reasoning_parts.append(f"Random forest technical analysis result: {signals['rf_signal']}, confidence: {signals['rf_confidence']:.2%}")
...
def preprocess_stock_data(price_df: pd.DataFrame, technical_indicators: Optional[Dict] = None) -> pd.DataFrame:
    ...
    if not all(col in price_df.columns for col in required_cols):
        raise ValueError(f"Price data must contain the following columns: {required_cols}")
