...
def extract_agent_data(
    state: Dict[str, Any], agent_name: str
...
def print_summary_report(state: Dict[str, Any]) -> None:
    ...
    reasoning = safe_get(
        final_decision, "reasoning", default="No decision reasoning provided"
    )
    ...
    print("\n" + "=" * width)
    print(
        f"{COLORS['bold']}{COLORS['bg_blue']}{'':^10}Stock Code {ticker} Investment Analysis Summary Report{'':^10}{COLORS['reset']}"
    )
    print("=" * width)

    # Print basic information
    print(
        f"{COLORS['bold']}Analysis Period:{COLORS['reset']} {start_date} to {end_date}"
    )
    print(
        f"{COLORS['bold']}Analysis Date:{COLORS['reset']} {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("-" * width)
    ...
    print(
        f"{COLORS['bold']}Confidence:{COLORS['reset']} {format_confidence(confidence)}"
    )
    ...
    if isinstance(reasoning, list):
        ...
        print(
            f"{COLORS['bold']}Reasoning:{COLORS['reset']} {reasoning[:200]}..."
            if len(str(reasoning)) > 200
            else f"{COLORS['bold']}Reasoning:{COLORS['reset']} {reasoning}"
        )
    ...
    if ai_model_data:
        ...
        if model_signals:
            ...
            for model_name, model_data in model_signals.items():
                ...
                print(
                    f"  • {model_name}: {get_signal_color(model_signal)} (Confidence: {format_confidence(model_conf)})"
                )

        # Print portfolio allocation recommendations (if any)
        if (
            safe_get(ai_model_data, "multi_asset")
            and "portfolio_allocation" in ai_model_data
        ):
            allocation = safe_get(
                ai_model_data, "portfolio_allocation", "allocation", default={}
            )
            if allocation:
                print(
                    f"{COLORS['bold']}Asset Allocation Recommendations:{COLORS['reset']}"
                )
    ...
    if valuation_data:
        ...
        if valuation_gap is not None:
            print(
                f"{COLORS['bold']}Valuation Gap:{COLORS['reset']} {format_confidence(valuation_gap)}"
            )
        ...
        if isinstance(reasoning, dict):
            for category in [
                ...
                if category_data:
                    ...
                    print(
                        f"{COLORS['bold']}{category_name}:{COLORS['reset']} {safe_get(category_data, 'signal', default='Unknown')}"
                    )
    ...
    if technical_data:
        ...
        if strategy_signals:
            ...
            for strategy_name, strategy_data in strategy_signals.items():
                ...
                print(
                    f"  • {strategy_name}: {get_signal_color(strategy_signal)} (Confidence: {format_confidence(strategy_conf)})"
                )
    ...
    if sentiment_data:
        ...
        reasoning = safe_get(
            sentiment_data, "reasoning", default="No reasoning provided"
        )
    ...
    if bull_data or bear_data:
        ...
        if bull_data:
            ...
            print(
                f"{COLORS['bold']}Bullish Perspective (Confidence: {format_confidence(bull_conf)}):{COLORS['reset']}"
            )
        ...
        if bear_data:
            ...
            print(
                f"{COLORS['bold']}Bearish Perspective (Confidence: {format_confidence(bear_conf)}):{COLORS['reset']}"
            )
    ...
    if debate_data:
        ...
        print(
            f"{COLORS['bold']}Bull Confidence:{COLORS['reset']} {format_confidence(bull_conf)}"
        )
        print(
            f"{COLORS['bold']}Bear Confidence:{COLORS['reset']} {format_confidence(bear_conf)}"
        )
        ...
        if ai_contribution and safe_get(ai_contribution, "included"):
            ...
            print(
                f"{COLORS['bold']}AI Model Contribution:{COLORS['reset']} {get_signal_color(ai_signal)} (Confidence: {format_confidence(ai_confidence)}, Weight: {ai_weight*100:.0f}%)"
            )
    ...
    if risk_data:
        ...
        print(
            f"{COLORS['bold']}Trading Recommendation:{COLORS['reset']} {get_signal_color(trading_action)}"
        )
        ...
        if garch_model and "forecast" in garch_model:
            print(
                f"{COLORS['bold']}Volatility Forecast:{COLORS['reset']} Future volatility trend {'rising' if safe_get(garch_model, 'forecast_annualized', 0) > safe_get(risk_metrics, 'volatility', 0) else 'falling'}"
            )
    ...
    if macro_data:
        ...
        if multi_asset:
            ...
            if tickers:
                print(
                    f"{COLORS['bold']}Multi-Asset Analysis ({len(tickers)} stocks):{COLORS['reset']}"
                )
                # Show risk diversification tips
                diversification_tips = safe_get(
                    multi_asset, "diversification_tips", default=[]
                )
    ...
    if portfolio_analysis_data:
        ...
        portfolio_analysis = safe_get(
            portfolio_analysis_data, "portfolio_analysis", default={}
        )
        risk_analysis = safe_get(portfolio_analysis_data, "risk_analysis", default={})
        efficient_frontier = safe_get(
            portfolio_analysis_data, "efficient_frontier", default={}
        )
        ...
        if "max_sharpe" in portfolio_analysis:
            ...
            print(
                f"{COLORS['bold']}Optimal Portfolio (Max Sharpe Ratio):{COLORS['reset']}"
            )
        ...
        if risk_analysis:
            ...
            if best_asset and worst_asset:
                print(
                    f"  • Best Asset: {safe_get(best_asset, 'ticker', default='Unknown')} (Return: {format_confidence(safe_get(best_asset, 'return', default=0))})"
                )
                print(
                    f"  • Worst Asset: {safe_get(worst_asset, 'ticker', default='Unknown')} (Return: {format_confidence(safe_get(worst_asset, 'return', default=0))})"
                )
    ...
    print("\n" + "=" * width)
    print(
        f"{COLORS['bold']}{COLORS['bg_green']}{'':^15}Analysis Report Complete{'':^15}{COLORS['reset']}"
    )
    print("=" * width + "\n")
...
def print_compact_summary(state: Dict[str, Any]) -> None:
    ...
    action_color = get_signal_color(action)
    print(
        f"{COLORS['bold']}Decision:{COLORS['reset']} {action_color} {quantity} shares"
    )
    ...
    for agent_name, display_name in agents_to_check:
        ...
        if agent_data:
            ...
            signals_summary.append(
                f"{display_name}: {get_signal_color(signal)} ({format_confidence(confidence)})"
            )
    ...
    for agent_name, display_name in AGENT_NAMES.items():
        ...
        if agent_data:
            ...
            if agent_name == "valuation_agent":
                agent_info["valuation_gap"] = safe_get(
                    agent_data, "valuation_gap", default=None
                )
                ...
                agent_info["max_position_size"] = safe_get(
                    agent_data, "max_position_size", default=0
                )
                agent_info["trading_action"] = safe_get(
                    agent_data, "trading_action", default="Unknown"
                )
                agent_info["risk_metrics"] = safe_get(
                    agent_data, "risk_metrics", default={}
                )

            elif agent_name == "ai_model_analyst_agent":
                agent_info["model_signals"] = safe_get(
                    agent_data, "model_signals", default={}
                )
                agent_info["multi_asset"] = safe_get(
                    agent_data, "multi_asset", default=False
                )
                if agent_info["multi_asset"]:
                    agent_info["portfolio_allocation"] = safe_get(
                        agent_data, "portfolio_allocation", default={}
                    )
                    agent_info["asset_signals"] = safe_get(
                        agent_data, "asset_signals", default={}
                    )

            elif agent_name == "portfolio_analyzer_agent":
                agent_info["portfolio_analysis"] = safe_get(
                    agent_data, "portfolio_analysis", default={}
                )
                agent_info["risk_analysis"] = safe_get(
                    agent_data, "risk_analysis", default={}
                )
