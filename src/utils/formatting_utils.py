...
def format_market_data_summary(market_data: Dict) -> Dict:
    ...
    if "market_returns" in market_data:
        if isinstance(market_data["market_returns"], str):
            ...
            formatted_data["market_returns_mean"] = float(
                market_data["market_returns"].mean()
            )
            formatted_data["market_returns_std"] = float(
                market_data["market_returns"].std()
            )
    ...
    if "stock_returns" in market_data:
        if isinstance(market_data["stock_returns"], str):
            ...
            formatted_data["stock_returns_mean"] = float(
                market_data["stock_returns"].mean()
            )
            formatted_data["stock_returns_std"] = float(
                market_data["stock_returns"].std()
            )
...
def format_currency(
    value: float, decimal_places: int = 2, currency_symbol: str = "$"
) -> str:
