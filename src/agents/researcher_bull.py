...
@agent_endpoint(
    "researcher_bull",
    "Bullish researcher, analyzing market data from a bullish perspective and proposing investment theses",
)
def researcher_bull_agent(state: AgentState):
    ...
    final_confidence = min(
        base_confidence + market_opportunity_adjustment, 0.9
    )  # Cap at 90%

    # Add opportunity adjustment reasoning
    if market_opportunity_adjustment > 0:
        bullish_points.append(
            f"Market opportunity factor analysis increases bullish confidence: +{market_opportunity_adjustment:.2f}"
        )
    ...
    message_content = {
        ...
        "opportunity_factors": {
            ...
            "macro_environment": (
                macro_data.get("macro_environment") if macro_data else None
            ),
            "volatility_regime": (
                risk_data.get("risk_metrics", {}).get("volatility_regime")
                if risk_data
                else None
            ),
        },
...
def analyze_bullish_fundamental(fundamental_signals: dict) -> dict:
    ...
    if signal == "bullish":
        ...
        if reasoning:
            ...
            if "growth" in str(reasoning).lower() and (
                "positive" in str(reasoning).lower()
                or "increas" in str(reasoning).lower()
            ):
                growth_indicators.append("Strong growth momentum")
            # Check profitability quality
            if "profitability" in str(reasoning).lower() and (
                "improv" in str(reasoning).lower() or "strong" in str(reasoning).lower()
            ):
                growth_indicators.append("Enhanced profitability")
            # Check cash flow
            if "cash" in str(reasoning).lower() and (
                "flow" in str(reasoning).lower()
                and "positive" in str(reasoning).lower()
            ):
...
def analyze_market_opportunity_factors(
    fundamental_signals: dict,
    technical_signals: dict,
    valuation_signals: dict,
    risk_data: dict = None,
    ...
    if risk_data:
        ...
        volatility_regime = risk_data.get("risk_metrics", {}).get(
    ...
    if (
        fundamental_signals.get("reasoning", {}).get("growth_signal", {}).get("signal")
        == "bullish"
    ):
        opportunity_adjustment += 0.1
    ...
    if (
        technical_signals.get("signal") == "neutral"
        and technical_signals.get("confidence", 0) > 0.7
    ):
    ...
    if (
        technical_signals.get("signal") == "bearish"
        and technical_signals.get("confidence", 0) > 0.7
    ):
