...
@agent_endpoint(
    "portfolio_analyzer",
    "Portfolio analyst, analyzing multi-asset portfolio performance, conducting portfolio optimization and risk assessment",
)
def portfolio_analyzer_agent(state: AgentState):
    ...
    try:
        ...
        portfolio_analysis = analyze_portfolio(
            tickers, returns_df, cov_matrix, expected_returns, risk_free_rate
        )
        ...
        ef_results = generate_efficient_frontier(
        ...
        message_content = {
            ...
            "summary": generate_summary(
                portfolio_analysis, risk_analysis, beta_analysis
            ),
        }
    ...
    return {
        "messages": [message],
        "data": {**data, "portfolio_analysis": message_content},
        "metadata": state["metadata"],
    }


def analyze_portfolio(
    tickers, returns_df, cov_matrix, expected_returns, risk_free_rate=0.03
):
    """
    ...
    equal_sharpe = portfolio_sharpe_ratio(
        equal_weights.values, expected_returns.values, cov_matrix.values, risk_free_rate
    )
...
def analyze_portfolio_risk(returns_df):
    ...
    return {
        ...
        "best_asset": {"ticker": best_asset, "return": float(mean_returns.max())},
        "worst_asset": {"ticker": worst_asset, "return": float(mean_returns.min())},
    }
...
def generate_efficient_frontier(
    expected_returns, cov_matrix, risk_free_rate=0.03, points=20
):
    """
    Generate efficient frontier
    """
...
def generate_summary(portfolio_analysis, risk_analysis, beta_analysis):
    ...
    summary.append(
        f"This allocation has expected annualized return of {max_sharpe['return']*100:.2f}%, volatility of {max_sharpe['risk']*100:.2f}%, and Sharpe ratio of {max_sharpe['sharpe_ratio']:.2f}"
    )

    # Risk assessment
    summary.append(
        f"Risk Assessment: 95% confidence level VaR is {var_95*100:.2f}%, maximum drawdown is {max_drawdown*100:.2f}%"
    )
    ...
    for ticker, beta_data in beta_analysis.items():
        if "average_beta" in beta_data:
            summary.append(
                f"- {ticker}: {beta_data['average_beta']:.2f} (latest: {beta_data['latest_beta']:.2f})"
            )
