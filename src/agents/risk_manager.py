...
@agent_endpoint(
    "risk_management",
    "Risk management expert, assessing investment risks and providing risk-adjusted trading recommendations",
)
def risk_management_agent(state: AgentState):
    ...
    logger.info(
        f"Risk manager detected market regime: {current_regime.get('regime_name', 'unknown')} (confidence: {current_regime.get('confidence', 0):.2f})"
    )
        ...
        if len(returns_dict) > 1:
            ...
            for i in range(len(correlation_matrix.columns)):
                for j in range(i + 1, len(correlation_matrix.columns)):
                    if (
                        correlation_matrix.iloc[i, j] > 0.7
            ...
            if high_corr_pairs:
                diversification_tips.append(
                    f"Found high correlation asset pairs: {', '.join(high_corr_pairs)}, consider reducing allocation of one asset to lower concentration risk"
                )
            ...
            if avg_corr > 0.6:
                diversification_tips.append(
                    f"Portfolio average correlation is high ({avg_corr:.2f}), may need to add other asset classes to improve diversification"
                )
    ...
    volatility_percentile = (
        (volatility - volatility_mean) / volatility_std if volatility_std != 0 else 0
    )
    ...
    if current_position_value > 0:
        for scenario, decline in stress_test_scenarios.items():
            ...
            scenario_var = potential_loss * (
                var_95 / 0.05
            )  # Adjust VaR to reflect scenario severity
    ...
    if macro_message:
        try:
            ...
            if macro_impact == "positive" and macro_environment == "positive":
                ...
                logger.info(
                    f"Reduced risk score to {risk_score} based on positive macro environment"
                )
        except Exception as e:
    ...
    if recent_period > 5:  # Ensure sufficient data
        ...
        if recent_positive_ratio > 0.65:  # More than 65% of days have positive returns
            ...
            logger.info(
                f"Reduced risk score to {risk_score} based on recent positive market performance"
            )
    ...
    if risk_score >= 9:
        ...
        if (
            debate_signal == "bearish" and debate_confidence >= 0.4
        ):  # Adjusted: was 0.3, now raised to 0.4
            trading_action = "sell"
            # Reason: Medium-high risk (6+/10) and strong bearish signal,
            # Need higher confidence to execute sell
        elif (
            debate_signal == "bullish" and debate_confidence >= 0.3
        ):  # Adjusted: was 0.4, now lowered to 0.3
            trading_action = "buy"
            # Reason: Medium-high risk but strong bullish signal,
            # Lower confidence requirement for buying, increase trading opportunities
        else:
            trading_action = (
                "buy"  # Adjusted: changed to buy instead of reduce, more aggressive
            )
        ...
        if (
            debate_signal == "bearish" and debate_confidence >= 0.3
        ):  # Adjusted: was 0.25, raised to 0.3
            trading_action = "sell"
            ...
            debate_signal == "bullish" and debate_confidence >= 0.25
        ):  # Adjusted: was 0.35, lowered to 0.25
            trading_action = "buy"
            # Reason: Medium risk with bullish signal, lower threshold to increase trading opportunities
        else:
            trading_action = (
                "buy"  # Adjusted: changed to buy instead of hold, more aggressive
            )
        ...
        if (
            debate_signal == "bullish" and debate_confidence >= 0.2
        ):  # Adjusted: was 0.3, lowered to 0.2
            trading_action = "buy"
            # Reason: Low risk environment with bullish signal, significantly lower threshold

        elif (
            debate_signal == "bearish" and debate_confidence >= 0.4
        ):  # Adjusted: was 0.35, raised to 0.4
            trading_action = "sell"
            # Reason: In low risk environment, need stronger bearish signal to sell

        elif (
            debate_signal == "neutral" and debate_confidence >= 0.25
        ):  # Adjusted: was 0.3, lowered to 0.25
    ...
    try:
        ...
        if len(returns) >= 100:  # Ensure sufficient data
            ...
            regime_risk_score += (
                1 if garch_params["persistence"] > 0.95 else 0
            )  # High persistence indicates higher risk
        ...
        "risk_metrics": {
            ...
            "macro_environment_assessment": {
                ...
                "volatility_regime": (
                    "high"
                    if volatility > 0.3
                    else "medium" if volatility > 0.2 else "low"
                ),
            },
        },
        "position_sizing": {
    ...
    if portfolio_risk_analysis:
        ...
        if (
            "diversification_tips" in portfolio_risk_analysis
            and portfolio_risk_analysis["diversification_tips"]
        ):
            message_content["reasoning"] += "\nMulti-asset Risk Analysis: " + " ".join(
                portfolio_risk_analysis["diversification_tips"]
            )
...
def _generate_regime_aware_diversification_tips(
    correlation_matrix, current_regime, high_corr_pairs
):
    ...
    if regime_name == "crisis_regime":
        tips.append(
            "In crisis regime, traditional diversification effects weaken, consider increasing safe-haven assets (bonds, gold) allocation"
        )
        tips.append(
            "During crisis periods, asset correlations tend toward 1, suggest reducing overall risk exposure"
        )
    elif regime_name == "high_volatility_mean_reverting":
        tips.append(
            "In high volatility oscillating markets, can appropriately increase hedging strategies and volatility trading"
        )
        tips.append(
            "Oscillating markets suit range trading, suggest setting stricter stop-loss and take-profit"
        )
    elif regime_name == "low_volatility_trending":
        tips.append(
            "Low volatility trending markets suit increasing momentum strategy allocation"
        )
        tips.append(
            "In trending markets, can appropriately increase single-direction exposure"
        )

    return tips


def _calculate_regime_risk_score(
    current_regime, volatility_percentile, var_95, max_drawdown, skewness, kurtosis
):
...
def _calculate_regime_aware_position_sizing(
    debate_results, current_regime, returns, total_portfolio_value
):
    ...
    avg_loss = (
        abs(returns[returns < 0].mean()) if len(returns[returns < 0]) > 0 else 0.005
    )
    ...
    # Final position size
    max_position_size = (
        total_portfolio_value * kelly_fraction * conservative_factor * risk_adjustment
    )
    max_position_size = max(
        max_position_size, total_portfolio_value * 0.02
    )  # Set minimum position
    max_position_size = min(
        max_position_size, total_portfolio_value * 0.4
