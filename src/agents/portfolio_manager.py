...
@agent_endpoint(
    ...
    "Responsible for portfolio management and final trading decisions",
)
def portfolio_management_agent(state: AgentState):
    ...
    if ai_model_message:
        try:
            ...
            if (
                ai_model_results.get("multi_asset", False)
                and "portfolio_allocation" in ai_model_results
            ):
                allocation = ai_model_results["portfolio_allocation"].get(
                    "allocation", {}
                )
                if allocation:
                    user_message[
...
def optimize_portfolio_decision_advanced(
    llm_decision,
    portfolio,
    current_price,
    risk_message,
    ...
    try:
        ...
        max_position_size = (
            portfolio["cash"] / current_price * 0.25 if current_price > 0 else 0
        )
        risk_score = 5
        trading_action = "hold"
        volatility_model = {}
    ...
    market_data = {}
    market_volatility = (
        0.15  # Default value, only used when actual data cannot be obtained
    )
    ...
    try:
        ...
        if "000300" in market_returns_dict:
            ...
            logger.warning(
                "Unable to obtain CSI 300 index data, using default market volatility"
            )
    except Exception as e:
    ...
    try:
        ...
        if not risk_free_data.empty:
            ...
            logger.info(
                f"Successfully obtained risk-free rate data: {risk_free_rate:.2%}"
            )
        else:
            # If data is empty, use default value
            risk_free_rate = 0.03
            logger.warning(
                f"Risk-free rate data is empty, using default value: {risk_free_rate:.2%}"
            )
    except Exception as e:
        # If acquisition fails, use reasonable default value
        risk_free_rate = 0.03
        logger.error(
            f"Failed to obtain risk-free rate: {e}, using default value: {risk_free_rate:.2%}"
        )
    ...
    if not prices_df.empty:
        try:
            ...
            logger.info(
                f"Successfully calculated stock volatility: {stock_volatility:.2%}"
            )
        except Exception as e:
    ...
    if ai_model_message:
        try:
            ...
            logger.info(
                f"Successfully parsed AI model analysis data: {ai_model_data.get('signal', 'neutral')}"
            )
        except Exception as e:
        ...
        if stock_returns is not None and len(stock_returns) > 30:
            ...
            tech_signal = next(
                (
                    ...
                    if s.get("agent_name", "").lower() == "technical_analysis"
                ),
                None,
            )
            fund_signal = next(
                (
                    s
                    for s in agent_signals
                    if s.get("agent_name", "").lower() == "fundamental_analysis"
                ),
                None,
            )

            if tech_signal and fund_signal:
                tech_value = {"bullish": 1, "neutral": 0, "bearish": -1}.get(
                    tech_signal.get("signal", "neutral").lower(), 0
                )
                fund_value = {"bullish": 1, "neutral": 0, "bearish": -1}.get(
                    fund_signal.get("signal", "neutral").lower(), 0
                )
                signal_value = (
                    tech_value * 0.4 + fund_value * 0.6
                )  # Fundamental analysis has higher weight
                expected_return_multiplier += (
                    signal_value * 0.5
                )  # Adjust baseline expected return
            ...
            if ai_model_data:
                ...
                ai_value = {"bullish": 1, "neutral": 0, "bearish": -1}.get(
                    ai_signal.lower(), 0
                )

                # Use AI model signal to further adjust expected return multiplier
                ai_adjustment = (
                    ai_value * ai_confidence * 0.2
                )  # AI model contributes 20% adjustment
                expected_return_multiplier += ai_adjustment
                logger.info(
                    f"AI model contribution adjustment: {ai_adjustment:.2f}, adjusted expected return multiplier: {expected_return_multiplier:.2f}"
                )
                ...
                try:
                    ...
                    if isinstance(garch_forecast, list) and len(garch_forecast) > 0:
                        ...
                        volatility_adjustment = (
                            current_vol / avg_forecast if avg_forecast > 0 else 1.0
                        )
                        volatility_adjustment = min(
                            max(volatility_adjustment, 0.7), 1.3
            ...
            adjusted_expected_return = (
                stock_return_mean * expected_return_multiplier * 252
            )  # Annualized
            adjusted_volatility = (
                stock_return_std * np.sqrt(252) / volatility_adjustment
            )  # Annualized and adjusted

            # Use actual risk-free rate to calculate Sharpe ratio
            sharpe_ratio = (
                (adjusted_expected_return - risk_free_rate) / adjusted_volatility
                if adjusted_volatility > 0
                else 0
            )

            # Use actual calculated Beta for optimization adjustment
            beta_adjusted_return = risk_free_rate + beta * (
                adjusted_expected_return - risk_free_rate
            )
                    ...
                    if len(tickers) > 1 and all_stock_data:
                        ...
                        for symbol in tickers:
                            if symbol in all_stock_data:
                                stock_prices = prices_to_df(
                                    all_stock_data[symbol]["prices"]
                                )
                                if not stock_prices.empty:
                                    returns_dict[symbol] = (
                                        stock_prices["close"].pct_change().dropna()
                                    )
                        ...
                        if len(returns_dict) > 1:
                            ...
                            try:
                                ...
                                if ai_model_data and ai_model_data.get(
                                    "multi_asset", False
                                ):
                                    portfolio_allocation = ai_model_data.get(
                                        "portfolio_allocation", {}
                                    )
                                    allocation = portfolio_allocation.get(
                                        "allocation", {}
                                    )
                                    ...
                                    if allocation:
                                        ...
                                        if ai_portfolio_weights.sum() > 0:
                                            ai_portfolio_weights = (
                                                ai_portfolio_weights
                                                / ai_portfolio_weights.sum()
                                            )

                                            # Create AI model recommended portfolio
                                            ai_return = (
                                                expected_returns * ai_portfolio_weights
                                            ).sum()
                                            ai_risk = np.sqrt(
                                                np.dot(
                                                    ai_portfolio_weights,
                                            ...
                                            ai_sharpe = (
                                                (ai_return - risk_free_rate) / ai_risk
                                                if ai_risk > 0
                                                else 0
                                            )
                                ...
                                portfolio_optimization_results = {
                                    ...
                                    "min_risk": {
                                        ...
                                        "sharpe_ratio": float(
                                            min_risk_portfolio["sharpe_ratio"]
                                        ),
                                    },
                                }

                                # Use maximum Sharpe ratio portfolio data
                                adjusted_expected_return = max_sharpe_portfolio[
                                    "return"
                                ]
                                ...
                                logger.error(
                                    f"Multi-asset portfolio optimization failed: {e}"
                                )
                        else:
                            # Single asset optimization logic
                            weights = np.array(
                                [1.0]
                            )  # 100% invested in this single asset
    ...
    kelly_fraction = (
        confidence * 2 - 1
    )  # Convert confidence to Kelly fraction (-1 to 1)
    ...
    if "sharpe_ratio" in portfolio_optimization_results:
        ...
        if sharpe_ratio > 1.0:  # Very good risk-adjusted return
            ...
            )  # Increase position but not exceed 1
    ...
    if ai_model_data:
        ...
        if action == "buy" and ai_signal == "bullish":
            ...
            kelly_fraction = min(kelly_fraction * (1 + ai_confidence * 0.2), 1.0)
        elif (action == "buy" and ai_signal == "bearish") or (
            action == "sell" and ai_signal == "bullish"
        ):
    ...
    suggested_position_value = (
        total_portfolio_value * kelly_fraction * conservative_factor * risk_factor
    )
    ...
    macro_signal = next(
        (
            s
            for s in agent_signals
            if s.get("agent_name", "").lower() == "macro_analysis"
        ),
        None,
    )
    ...
    suggested_quantity = (
        int(suggested_position_value / current_price) if current_price > 0 else 0
    )
    ...
    if not prices_df.empty and portfolio["stock"] > 0:
        ...
        avg_price = (
            prices_df["close"].iloc[-recent_period:].mean()
            if recent_period > 0
            else current_price
        )
        if avg_price > 0:
    ...
    if action == "buy":
        ...
        max_affordable = (
            int(portfolio["cash"] / current_price) if current_price > 0 else 0
        )
        ...
        min_shares = (
            max(1, int(min_transaction / current_price)) if current_price > 0 else 1
        )
    ...
    if ai_model_data:
        ...
        if ai_signal == action.replace("hold", "neutral").replace(
            "buy", "bullish"
        ...
        if (
            ai_model_data.get("multi_asset", False)
        ...
        "ai_model_integration": (
            {
                ...
                "signal": (
                    ai_model_data.get("signal", "neutral") if ai_model_data else None
                ),
                "confidence": (
                    ai_model_data.get("confidence", 0) if ai_model_data else None
                ),
                "impact_on_position": (
