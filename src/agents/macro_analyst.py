...
@agent_endpoint(
    "macro_analyst",
    "Macro analyst, analyzing the impact of macroeconomic environment on target stocks",
)
def macro_analyst_agent(state: AgentState):
    ...
    if len(tickers) > 1:
        try:
            ...
            if all_assets_news:
                ...
                logger.info(
                    f"Multi-asset macro analysis completed: {multi_asset_analysis.get('macro_environment', 'neutral')}"
                )
    ...
    if multi_asset_analysis and len(tickers) > 1:
        message_content = {
            ...
            "confidence": (
                0.7
                if macro_analysis.get("impact_on_stock") == "positive"
                else (
            ...
            "reasoning": macro_analysis.get(
                "reasoning", "No macro analysis reasoning provided"
            ),
    ...
    return {
        "messages": [message],
        "data": {**data, "macro_analysis": message_content},
        ...
        try:
            ...
            if json_match:
                try:
                    ...
                    logger.info(
                        "Successfully extracted and parsed JSON results from code block"
                    )
                except:
