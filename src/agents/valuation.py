...
@agent_endpoint("valuation", "Valuation analyst, using DCF and owner earnings methods to assess company intrinsic value")
def valuation_agent(state: AgentState):
    ...
    try:
        ...
        if '000300' in market_returns_dict:
            ...
            logger.warning("Unable to get CSI 300 index data, will try other methods to get market data")
    ...
    try:
        ...
        logger.info(f"Obtained real market data: Market return={market_return:.2%}, Risk-free rate={risk_free_rate:.2%}")
    except Exception as e:
        # Use reasonable default values when unable to get data
        logger.warning(f"Unable to get market return rate and risk-free rate data: {e}, using default values")
    ...
    beta_value = calculate_beta(ticker, "000300", start_date, end_date)  # Use function from calc/calculate_beta.py
    logger.info(f"Calculated Beta value using improved Beta calculation function: {beta_value:.2f}")
    ...
    if market_volatility is None:
        if not prices_df.empty:
            try:
                ...
                logger.info(f"Reverse calculated market volatility using portfolio_volatility and beta: {market_volatility:.2%}")
            except Exception as e:
    ...
    try:
        ...
        logger.info(f"Obtained industry data: PE={industry_avg_pe}, PB={industry_avg_pb}, Growth Rate={industry_growth}")
    except Exception as e:
    ...
    growth_premium = max(0, company_growth - industry_growth) * 10  # Give 10 PE premium for each 1% additional growth
    ...
    consistency_boost = max(0, 1 - signal_variance * 0.5)  # 50% boost when variance is 0
    confidence = min(confidence * (1 + consistency_boost * 0.5), 1.0)  # Maximum 50% boost
...
def calculate_owner_earnings_value(
    ...
    try:
        ...
        terminal_growth = min(growth_rate * 0.4, 0.03)  # Take 40% of growth rate or 3%, whichever is smaller
        terminal_value = (
...
def calculate_multistage_dcf(
    ...
    try:
        ...
        reliability_factor = min(max(net_margin / 0.15, 0.5), 1.5)  # Ratio of net margin to industry average (15%)
