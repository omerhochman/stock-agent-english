...
@agent_endpoint(
    "ai_model_analyst",
    "AI model analyst, running deep learning, reinforcement learning and genetic programming models for prediction",
)
def ai_model_analyst_agent(state: AgentState):
            ...
            try:
                if ticker in all_stock_data:
                    ...
                    if not asset_prices_df.empty:
                        ...
                        # Combine signals
                        combined_signal = combine_ai_signals(
                            ml_signals, rl_signals, factor_signals
                        )
    ...
    return {
        "messages": [message],
        "data": {**data, "ai_analysis": message_content},
        "metadata": state["metadata"],
    }
...
def run_deep_learning_model(prices_df):
    ...
    try:
        ...
        return {
            ...
            "reasoning": signals.get(
                "reasoning",
                "Deep learning model based on LSTM and Random Forest algorithm prediction",
            ),
        }
    except Exception as e:
...
def run_reinforcement_learning_model(prices_df):
    ...
    try:
        ...
        return {
            ...
            "reasoning": signals.get(
                "reasoning",
                "Reinforcement learning model based on PPO algorithm optimizing trading decisions",
            ),
        }
    except Exception as e:
...
def run_genetic_programming_model(prices_df):
    ...
    try:
        ...
        return {
            ...
            "reasoning": signals.get(
                "reasoning",
                "Genetic programming factor model analyzes market through automated factor mining",
            ),
        }
    except Exception as e:
...
def combine_ai_signals(ml_signals, rl_signals, factor_signals):
    ...
    ml_score = (
        signal_values.get(ml_signals["signal"], 0)
        * ml_signals["confidence"]
        * weights["deep_learning"]
    )
    rl_score = (
        signal_values.get(rl_signals["signal"], 0)
        * rl_signals["confidence"]
        * weights["reinforcement_learning"]
    )
    factor_score = (
        signal_values.get(factor_signals["signal"], 0)
        * factor_signals["confidence"]
        * weights["genetic_programming"]
    )
...
def analyze_portfolio_ai_signals(all_asset_signals):
    ...
    avg_confidence = (
        total_confidence / len(all_asset_signals) if all_asset_signals else 0.5
    )
    ...
    reasoning += (
        f"{signal_counts['bearish']} bearish, and {signal_counts['neutral']} neutral.\n"
    )
