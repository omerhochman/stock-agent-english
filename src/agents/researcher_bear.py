...
@agent_endpoint(
    "researcher_bear",
    "Bearish researcher, analyzing market data from a bearish perspective and proposing risk warnings",
)
def researcher_bear_agent(state: AgentState):
    ...
    if market_risk_adjustment > 0:
        bearish_points.append(
            f"Market risk factor analysis increases bearish confidence: +{market_risk_adjustment:.2f}"
        )
...
def analyze_bearish_fundamental(fundamental_signals: dict) -> dict:
    ...
    if signal == "bearish":
        ...
        if reasoning:
            ...
            if (
                "cash_flow" in str(reasoning).lower()
                and "negative" in str(reasoning).lower()
            ):
                risk_indicators.append("Deteriorating cash flow")
            # Check debt issues
            if "debt" in str(reasoning).lower() and (
                "high" in str(reasoning).lower() or "increase" in str(reasoning).lower()
            ):
                risk_indicators.append("Increasing debt burden")
            # Check declining profit margins
            if "margin" in str(reasoning).lower() and (
                "declin" in str(reasoning).lower()
                or "compress" in str(reasoning).lower()
            ):
...
def analyze_market_risk_factors(
    ...
    if (
        fundamental_signals.get("reasoning", {})
        .get("financial_health_signal", {})
        .get("signal")
        == "bearish"
    ):
        risk_adjustment += 0.1
