...
def analyze_asset_correlations(returns_df: pd.DataFrame) -> Dict:
    ...
    if avg_corr > 0.6:
        diversification_tips.append("Portfolio has high overall correlation, may need to add low-correlation assets to improve diversification")
    
    if high_corr_pairs:
        diversification_tips.append("Consider reducing one of the high-correlation asset pairs to avoid redundant risk")
    
    if len(low_corr_pairs) < len(correlation.columns) / 4:
        diversification_tips.append("Portfolio has few low-correlation assets, consider adding other industries or asset classes")
    ...
    try:
        ...
        if explained_variance > 0.8:
            diversification_tips.append(f"First 3 principal components explain {explained_variance:.1%} of variance, indicating most risk comes from few sources, need better risk diversification")
    except ImportError:
    ...
    try:
        ...
        if result['success']:
            ...
            return {
                ...
                'weights': {col: 1.0/n_assets for col in returns_df.columns}  # Default equal weights
            }
