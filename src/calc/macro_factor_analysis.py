...
def analyze_macro_factor_exposures(
    returns_df: pd.DataFrame, macro_factors_df: pd.DataFrame
) -> Dict:
    """
    ...
    # Results dictionary
    results = {"success": True, "exposures": {}, "sensitivity_summary": []}
    ...
    for asset in returns_df.columns:
        ...
        try:
            ...
            significant_factors = {
                factor: {"coefficient": float(coef), "p_value": float(pvalues[factor])}
            ...
            if significant_factors:
                for factor, data in significant_factors.items():
                    sensitivity = (
                        ...
                        else "medium" if abs(data["coefficient"]) > 0.2 else "low"
                    )
...
def create_macro_factor_portfolio(
    returns_df: pd.DataFrame,
    macro_factors_df: pd.DataFrame,
    target_factor: str,
    exposure_direction: str = "positive",
) -> Dict:
    """
    ...
    for asset, exposure_data in exposures_analysis["exposures"].items():
        if (
            "all_coefficients" in exposure_data
            and target_factor in exposure_data["all_coefficients"]
        ):
            factor_sensitivities[asset] = exposure_data["all_coefficients"][
                target_factor
            ]
    ...
    if exposure_direction == "positive":
        selected_assets = {
            asset: sens for asset, sens in factor_sensitivities.items() if sens > 0
        }
    else:  # 'negative'
        selected_assets = {
            asset: sens for asset, sens in factor_sensitivities.items() if sens < 0
    ...
    if total_sensitivity > 0:
        weights = {
            asset: abs(sens) / total_sensitivity
            for asset, sens in selected_assets.items()
        }
    else:
    ...
    portfolio_exposure = sum(
        sens * weights[asset] for asset, sens in selected_assets.items()
    )
    ...
    return {
        ...
        "asset_sensitivities": {
            asset: float(sens) for asset, sens in selected_assets.items()
        },
    }


def analyze_factor_rotation(
    returns_df: pd.DataFrame, macro_factors_df: pd.DataFrame, window: int = 60
) -> Dict:
    """
    ...
    # Results dictionary
    results = {"success": True, "rotation_analysis": {}, "trend_summary": []}
            ...
            if (
                ...
                try:
                    ...
                    factor_trends[factor] = {
                        ...
                        "change": float(
                            factor_sensitivities[-1] - factor_sensitivities[0]
                        ),
                    }
