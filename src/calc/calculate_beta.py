def calculate_beta(ticker, market_index="000300", start_date=None, end_date=None):
    ...
    from src.tools.factor_data_api import get_index_data, get_market_returns
    ...
    try:
        ...
        if stock_prices is None or len(stock_prices) == 0:
            logger.warning(
                f"Unable to get stock {ticker} price data, using default Beta value"
            )
        ...
        try:
            ...
            market_data = get_market_returns(
                index_code=market_index, start_date=start_date, end_date=end_date
            )
            if market_data is None or len(market_data) == 0:
                logger.info(
                    f"Unable to get market data from factor_data_api, trying alternative approach"
                )
                # Alternative approach: directly get index prices and calculate returns
                market_prices = get_index_data(
                ...
                if market_prices is not None and not market_prices.empty:
                    ...
                    logger.warning(
                        f"Unable to get market index {market_index} data, using default Beta value"
                    )
            ...
            try:
                ...
                if market_prices is None or len(market_prices) == 0:
                    logger.warning(
                        f"Unable to get market index data through price API, using default Beta value"
                    )
                    return 1.0
                ...
                logger.error(
                    f"Alternative method to get market data also failed: {subex}"
                )
        ...
        try:
            ...
            # Debug output
            logger.info(
                f"Stock date range: {stock_returns.index.min()} to {stock_returns.index.max()}"
            )
            logger.info(
                f"Market date range: {market_returns.index.min()} to {market_returns.index.max()}"
            )
            logger.info(f"Number of common dates: {len(common_dates)}")

            if len(common_dates) < 15:  # Need sufficient data points
                logger.warning(
                    f"Insufficient overlap between stock and market data, only {len(common_dates)} common dates, using default Beta value"
                )
                return 1.0
            ...
            if market_variance > 0:
                ...
                logger.info(
                    f"Successfully calculated Beta value for {ticker}: {beta:.2f}"
                )
            else:
                logger.warning(
                    f"Market variance is zero, cannot calculate Beta value, using default value"
                )
                beta = 1.0
            ...
            if not (0.0 <= beta <= 3.0):
                logger.warning(
                    f"Calculated Beta value {beta:.2f} exceeds reasonable range, adjusting to limit range"
                )
