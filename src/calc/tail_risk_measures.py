...
def calculate_historical_var(
    returns: pd.Series, confidence_level: float = 0.95, window: int = None
) -> float:
    """
...
def calculate_parametric_var(
    returns: pd.Series, confidence_level: float = 0.95, distribution: str = "normal"
) -> float:
    """
...
def backtesting_var(
    ...
    if len(returns) <= window:
        raise ValueError(
            f"Not enough data for backtesting. Need more than {window} observations."
        )
    ...
    if violations > 0:
        kupiec_stat = -2 * (
            np.log(
                (1 - expected_rate) ** (len(returns) - window - violations)
                * expected_rate**violations
            )
            - np.log(
                (1 - violation_rate) ** (len(returns) - window - violations)
