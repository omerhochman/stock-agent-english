...
def calculate_fama_french_factors_tushare(start_date: str, end_date: str, freq: str = 'W') -> Dict[str, pd.Series]:
    ...
    """
    logger.info(f"Calculating Fama-French three-factor data using TuShare: {start_date} to {end_date}, frequency: {freq}")
    ...
    try:
        ...
        if market_df.empty:
            logger.warning("Unable to get market index data, trying daily frequency data and resampling")
            market_df = pro.index_daily(ts_code='000300.SH', start_date=start_date, end_date=end_date)
            if market_df.empty:
                logger.warning("Still unable to get market data, will use simulated data")
        ...
        logger.info(f"Successfully calculated Fama-French three factors, {len(factors_dict['market_returns'])} records")
        return factors_dict
...
def get_fama_french_factors(start_date: str = None, 
    ...
    Returns:
        Dictionary containing market risk premium, SMB, HML factors
    """
    logger.info(f"Getting Fama-French three-factor data: {start_date} to {end_date}, frequency: {freq}")
    ...
    if use_cache and os.path.exists(cache_file):
        try:
            ...
            logger.info(f"Successfully loaded Fama-French factor data from cache: {len(cache_data)} records")
        ...
        try:
            ...
            if os.environ.get('TUSHARE_TOKEN', ''):
                ...
                try:
                    ...
                    logger.warning(f"Failed to save Fama-French factor data to cache: {e}")
                
                return factors_dict
            else:
                logger.warning("TuShare token not found, cannot use TuShare to calculate factors")
        except ImportError:
            logger.warning("tushare library not found, cannot use TuShare to calculate factors")
...
def _generate_mock_fama_french_factors(start_date: str, end_date: str, freq: str = 'D') -> Dict[str, pd.Series]:
    ...
    Returns:
        Dictionary containing market risk premium, SMB, HML factors
    """
    logger.info(f"Generating simulated Fama-French three-factor data, frequency: {freq}")
