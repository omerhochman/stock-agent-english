...
def get_risk_free_rate(
    ...
    """
    logger.info(
        f"Getting risk-free rate data: {start_date} to {end_date}, frequency: {freq}"
    )
    ...
    if use_cache and os.path.exists(cache_file):
        try:
            ...
            logger.info(
                f"Successfully loaded risk-free rate data from cache: {len(cache_data)} records"
            )
        ...
        try:
            ...
            try:
                ...
                bond_data = bond_data.rename(
                    columns={
                        "date": "date",
                        "China 10-year Treasury Yield": "risk_free_rate",
                    }
                )
                ...
                bond_data["risk_free_rate"] = (
                    pd.to_numeric(bond_data["risk_free_rate"], errors="coerce") / 100
                )
        ...
        try:
            ...
            try:
                ...
                if not shibor_data.empty:
                    ...
                    shibor_data["risk_free_rate"] = (
                        pd.to_numeric(shibor_data["risk_free_rate"], errors="coerce")
                        / 100
                    )  # Convert to decimal
                    ...
                    if freq == "W":
                        shibor_data = (
                            shibor_data.resample("W-FRI").last().fillna(method="ffill")
                        )
                        # Convert annualized rate to weekly rate
                        shibor_data = shibor_data / 52
                    elif freq == "M":
                        shibor_data = (
                            shibor_data.resample("M").last().fillna(method="ffill")
                        )
                        # Convert annualized rate to monthly rate
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to save risk-free rate data to cache: {e}"
                        )
...
def _generate_mock_risk_free_rate(
