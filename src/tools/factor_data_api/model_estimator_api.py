...
def estimate_capm_for_stock(
    stock_symbol: str, start_date: str = None, end_date: str = None, freq: str = "D"
) -> Dict[str, float]:
    """
    ...
    """
    logger.info(
        f"Estimating CAPM model parameters for stock {stock_symbol}: {start_date} to {end_date}, frequency: {freq}"
    )
    ...
    try:
        ...
        if len(common_index) < 20:
            logger.warning(
                f"Too few data points to reliably estimate CAPM model: {len(common_index)} records"
            )
            return {}
        ...
        logger.info(
            f"Successfully estimated CAPM model parameters for stock {stock_symbol}"
        )
        return capm_results
...
def estimate_fama_french_for_stock(
    stock_symbol: str, start_date: str = None, end_date: str = None, freq: str = "D"
) -> Dict[str, float]:
    """
    ...
    """
    logger.info(
        f"Estimating Fama-French three-factor model parameters for stock {stock_symbol}: {start_date} to {end_date}, frequency: {freq}"
    )
    ...
    try:
        ...
        common_index = stock_returns.index.intersection(
            ff_factors["market_returns"].index
        )

        if len(common_index) < 20:
            logger.warning(
                f"Too few data points to reliably estimate Fama-French model: {len(common_index)} records"
            )
            return {}
        ...
        logger.info(
            f"Successfully estimated Fama-French three-factor model parameters for stock {stock_symbol}"
        )
        return ff_results
...
def estimate_beta_for_stocks(
    symbols: List[str], start_date: str = None, end_date: str = None, freq: str = "D"
) -> pd.DataFrame:
    """
    ...
    """
    logger.info(
        f"Estimating beta coefficients for multiple stocks: {start_date} to {end_date}, stock count: {len(symbols)}, frequency: {freq}"
    )
    ...
    for symbol in returns_df.columns:
        ...
        try:
            ...
            logger.warning(
                f"Error calculating beta coefficient for stock {symbol}: {e}"
            )
            betas[symbol] = np.nan
...
def calculate_rolling_beta(
    ...
    """
    logger.info(
        f"Calculating rolling beta coefficient for stock {stock_symbol}: {start_date} to {end_date}, window size: {window}"
    )
    ...
    if len(stock_returns) < window + 10:
        logger.warning(
            f"Insufficient data length, cannot calculate rolling beta: {len(stock_returns)} < {window + 10}"
        )
        return pd.Series()
    ...
    for i in range(window - 1, len(stock_returns)):
        ...
        try:
            model = sm.OLS(y, X).fit()
            rolling_betas.loc[stock_returns.index[i]] = model.params.iloc[
                1
            ]  # Beta coefficient
        except Exception as e:
            logger.debug(
                f"Error calculating beta coefficient for {stock_returns.index[i]}: {e}"
            )
    ...
    logger.info(
        f"Successfully calculated rolling beta coefficient for stock {stock_symbol}: {len(rolling_betas)} records"
    )
