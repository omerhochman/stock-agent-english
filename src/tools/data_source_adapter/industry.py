...
def _strip_suffix(name: str) -> str:
    ...
    return re.sub(
        r"(行业|Ⅱ|Ⅰ|板块)$", "", name
    ).strip()  # Remove 行业(industry), Ⅱ, Ⅰ, 板块(sector) suffixes
...
def get_industry_valuation(industry: str) -> tuple[float, float]:
    ...
    def _fetch_valuation():
        ...
        for fetch in fetchers:
            ...
            if not hit.empty:
                row = hit.iloc[0]
                return float(row["TTM(滚动)市盈率"]), float(
                    row["市净率"]
                )  # TTM P/E ratio, P/B ratio
        ...
        # ---------- 3) Still no match ----------
        raise ValueError(
            f"Could not find in Shenwan 1/2/3 level industries: {industry}"
        )
...
def get_industry_growth(industry: str, window_days: int = 252) -> float:
    ...
    logger.info(
        f"Get industry {industry} recent {window_days} day growth: {growth*100:.2f}%"
    )
    return growth
