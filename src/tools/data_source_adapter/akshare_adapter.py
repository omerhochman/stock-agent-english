...
def get_akshare_price_data(
    akshare_code: str, start_date: str, end_date: str, adjust: str
) -> pd.DataFrame:
...
def get_akshare_financial_metrics(
    akshare_code: str, exchange_prefix: str
) -> List[Dict[str, Any]]:
        ...
        if not realtime_data.empty:
            ...
            if not stock_data.empty:
                ...
                agent_metrics = {
                    ...
                    "pe_ratio": float(
                        stock_data.get("市盈率-动态", 0)
                    ),  # Dynamic P/E ratio
                    "price_to_book": float(
                        stock_data.get("市净率", 0)
                    ),  # Price-to-book ratio
                    "price_to_sales": 3.0,
                }
    ...
    try:
        ...
        if not stock_data.empty:
            ...
            hist_data = ak.stock_zh_a_hist(
                symbol=akshare_code, period="daily", adjust="qfq"
            )

            week_high = (
                hist_data["最高"].max()
                if not hist_data.empty
                else stock_data.get("最高", 0)
            )  # High
            week_low = (
                hist_data["最低"].min()
                if not hist_data.empty
                else stock_data.get("最低", 0)
            )  # Low
            avg_volume = (
                hist_data.tail(30)["成交量"].mean()
                if not hist_data.empty
                else stock_data.get("成交量", 0)
            )  # Volume
