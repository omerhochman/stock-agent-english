...
def format_df_to_markdown(
    df: pd.DataFrame,
    max_rows: int = MAX_MARKDOWN_ROWS,
    max_cols: int = MAX_MARKDOWN_COLS,
) -> str:
    ...
    if original_cols > max_cols:
        ...
        cols_to_show = (
            df_display.columns[: max_cols // 2].tolist()
        ...
        cols_to_show = sorted(
            list(set(cols_to_show)), key=list(df_display.columns).index
        )
    ...
    if truncated:
        ...
        logger.debug(f"Generated Markdown table with truncation notes: {notes}")
