...
from src.backtest.core import BacktestConfig, Backtester
...
def main():
    ...
    try:
        ...
        if not args.baseline_only:
            ...
            logger.info(
                f"✓ AI Agent backtest completed - Total return: {agent_result.performance_metrics.get('total_return', 0)*100:.2f}%"
            )
        ...
        for name, result in baseline_results.items():
            ...
            logger.info(
                f"✓ {name} - Return: {total_return:.2f}%, Sharpe: {sharpe_ratio:.3f}, Drawdown: {max_drawdown:.2f}%"
            )
        ...
        if comparison_results and "strategy_ranking" in comparison_results:
            ...
            if "by_sharpe" in ranking and ranking["by_sharpe"]:
                ...
                logger.info(
                    f"   Total Return: {best_strategy['total_return']*100:.2f}%"
                )
        ...
        if "broadcast" in error_msg:
            logger.error(
                "Suggestion: This might be a data dimension mismatch issue, please check data quality and length"
            )
        elif "feature_names_in_" in error_msg:
            logger.error(
                "Suggestion: This might be a model feature name issue, please check model training status"
            )
        elif "insufficient data" in error_msg:
            logger.error(
                "Suggestion: Please increase backtest time range or check data availability"
            )
...
Usage examples:
  ...
  python src/main_backtester.py --ticker 000001 --baseline-only --save-results --export-report
    ...
    # Required parameters
    parser.add_argument(
        "--ticker", type=str, required=True, help="Primary stock code (e.g.: 000001)"
    )
    ...
    parser.add_argument(
        "--num-of-news", type=int, default=5, help="Daily news count (default: 5)"
    )
    ...
    # Output options
    parser.add_argument(
        "--save-results", action="store_true", help="Save backtest results to file"
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed log information"
    )
    ...
    for comparison_key, result in comparisons.items():
        try:
            if "summary" in result:
                ...
                logger.info(
                    f"  {comparison_key}: {conclusion} (Power: {power:.3f}, {significance})"
                )
                if power > 0.5:
                    significant_count += 1
...
def _display_strategy_ranking(comparison_results: dict, logger):
    ...
    if "by_sharpe" in ranking and ranking["by_sharpe"]:
        ...
        for i, strategy in enumerate(ranking["by_sharpe"][:5], 1):
            logger.info(
                f"  {i}. {strategy['strategy']}: {strategy['sharpe_ratio']:.3f}"
            )
    ...
    if "by_return" in ranking and ranking["by_return"]:
        ...
        for i, strategy in enumerate(ranking["by_return"][:5], 1):
            logger.info(
                f"  {i}. {strategy['strategy']}: {strategy['total_return']*100:.2f}%"
            )
