...
class MomentumStrategy(BaseStrategy):
    ...
    def calculate_momentum(self, prices: pd.Series) -> float:
        ...
        momentum_6m = (
            (prices.iloc[-1] / prices.iloc[-126] - 1) if len(prices) >= 126 else 0
        )
        momentum_12m = (
            (prices.iloc[-1] / prices.iloc[-252] - 1) if len(prices) >= 252 else 0
        )
    ...
    def generate_signal(
        ...
        volatility = (
            prices.pct_change().rolling(21).std().iloc[-1] if len(prices) > 21 else 0
        )
        volume_trend = (
            (
                data["volume"].rolling(21).mean().iloc[-1]
                / data["volume"].rolling(63).mean().iloc[-1]
            )
            if len(data) > 63
            else 1
        )
        ...
        if momentum > self.momentum_threshold and position_ratio < 0.8:
            ...
            if volume_confirmation or not volatility_filter:
                ...
                if quantity > 0:
                    # Set holding period
                    self.hold_until_date = current_date_obj + pd.Timedelta(
                        days=self.holding_period
                    )
