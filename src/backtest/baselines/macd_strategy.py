    ...
    def detect_macd_divergence(self, prices: pd.Series, macd_line: pd.Series) -> str:
        ...
        for i in range(2, len(recent_prices) - 2):
            ...
            if (
                ...
                and recent_macd.iloc[i] > recent_macd.iloc[i - 2]
                and recent_macd.iloc[i] > recent_macd.iloc[i + 2]
            ):
        ...
        if len(price_troughs) >= 2 and len(macd_troughs) >= 2:
            ...
            if (
                latest_price_trough[1] < prev_price_trough[1]
                and latest_macd_trough[1] > prev_macd_trough[1]
            ):
                return "bullish"
        ...
        if len(price_peaks) >= 2 and len(macd_peaks) >= 2:
            ...
            if (
                latest_price_peak[1] > prev_price_peak[1]
                and latest_macd_peak[1] < prev_macd_peak[1]
            ):
                return "bearish"
