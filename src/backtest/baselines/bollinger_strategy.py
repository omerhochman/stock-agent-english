...
class BollingerStrategy(BaseStrategy):
    ...
    def calculate_bandwidth(
        self, upper_band: float, lower_band: float, middle_band: float
    ) -> float:
    ...
    def calculate_bb_position(
        self, price: float, upper_band: float, lower_band: float
    ) -> float:
    ...
    def detect_squeeze(self, prices: pd.Series) -> bool:
        ...
        return (
            current_bandwidth < self.squeeze_threshold
            and current_bandwidth <= avg_bandwidth
        )
    ...
    def calculate_volume_confirmation(self, data: pd.DataFrame) -> float:
        ...
        current_volume_ratio = (
            volume.iloc[-1] / volume_sma.iloc[-1] if volume_sma.iloc[-1] > 0 else 1.0
        )
    ...
    def generate_signal(
        ...
        volume_ratio = (
            self.calculate_volume_confirmation(data)
    ...
    def _breakout_signal(
        ...
        # Downward breakout sell signal
            ...
            quantity = min(
                portfolio.stock, int(portfolio.stock * min(0.7, signal_strength * 0.4))
            )
