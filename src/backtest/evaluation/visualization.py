...
class BacktestVisualizer:
    ...
    def _create_performance_comparison(self, results: Dict[str, Any], filepath: str) -> str:
        ...
        try:
            ...
            ax2.axhline(y=1.0, color='red', linestyle='--', alpha=0.7, label='Excellent Level')
            ax2.legend()
            ...
            logger.info(f"Successfully created performance comparison chart: {filepath}")
            return filepath
        ...
        try:
            ...
            for name, result in results.items():
                try:
                    ...
                    logger.warning(f"Error processing strategy {name} risk-return data: {e}")
                    continue
            ...
            ax.text(0.02, 0.98, 'Point size represents Sharpe ratio', 
                ...
                try:
                    ...
                    if len(daily_returns) > 0:
                        ...
                        axes[i].fill_between(dates, 0, drawdown, 
                                        color=colors[i], alpha=0.3, label='Drawdown')
                        ...
                        axes[i].annotate(f'Max Drawdown: {max_dd_value:.2f}%',
                                    xy=(dates[max_dd_idx], max_dd_value),
                                    xytext=(10, -10), textcoords='offset points',
                                    bbox=dict(boxstyle='round,pad=0.3', 
                                            facecolor='yellow', alpha=0.7))
                        
                        axes[i].set_title(f'{name} - Drawdown Analysis', fontsize=14, fontweight='bold')
                        axes[i].set_ylabel('Drawdown (%)', fontsize=12)
                        axes[i].grid(True, alpha=0.3)
                        axes[i].axhline(y=-5, color='orange', linestyle='--', 
                                    alpha=0.7, label='5% Drawdown Line')
                        axes[i].axhline(y=-10, color='red', linestyle='--', 
                                    alpha=0.7, label='10% Drawdown Line')
                        axes[i].legend()
                    ...
                    logger.warning(f"Error processing strategy {name} drawdown data: {e}")
                    continue
    ...
    def _create_strategy_radar_chart(self, ranking_data: Dict[str, Any], filepath: str) -> str:
        ...
        try:
            ...
            for i, strategy in enumerate(top_strategies):
                try:
                    ...
                    if hasattr(strategy, 'scores'):
                        ...
                        logger.warning(f"Strategy {getattr(strategy, 'name', 'Unknown')} has no scores data")
                        continue
            ...
            ax.set_title('Strategy Five-Dimension Radar Chart', fontsize=16, fontweight='bold', pad=20)
    ...
    def _create_rolling_metrics_chart(self, results: Dict[str, Any], filepath: str) -> str:
        ...
        try:
            ...
            if not valid_results:
                logger.warning("Insufficient data to create rolling metrics chart (need at least 60 days of data)")
                return ""
            ...
            for i, (name, result) in enumerate(valid_results):
                try:
                    ...
                    logger.warning(f"Error processing strategy {name} rolling metrics: {e}")
                    continue
            ...
            ax4.set_title('60-Day Rolling Maximum Drawdown', fontsize=14, fontweight='bold')
    ...
    def _create_monthly_returns_heatmap(self, results: Dict[str, Any], filepath: str) -> str:
        ...
        try:
            ...
            if selected_result is None:
                logger.warning("Insufficient data to generate monthly returns heatmap (need at least 30 days of data)")
                return ""
            ...
            if len(returns) < 30:
                logger.warning("Insufficient valid data to generate monthly returns heatmap")
                return ""
            ...
            try:
                ...
                sns.heatmap(pivot_table, annot=True, fmt='.2f', cmap='RdYlGn',
                        center=0, cbar_kws={'label': 'Monthly Returns (%)'}, ax=ax)
                ...
                ax.set_title(f'{strategy_name} - Monthly Returns Heatmap', fontsize=16, fontweight='bold')
