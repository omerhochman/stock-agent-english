...
class SignificanceTester:
    ...
    def _interpret_sharpe_test(self, sharpe1: float, sharpe2: float, p_value: float) -> str:
        ...
        if p_value >= self.confidence_level:
            return "No significant difference in Sharpe ratios between the two strategies"
    ...
    def _generate_comparison_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if results['paired_test']['significant']:
            ...
            significant_tests.append(f"Paired test: {winner} significantly outperforms the other strategy")
        
        if results['diebold_mariano']['significant']:
            significant_tests.append(f"DM test: {results['diebold_mariano']['interpretation']}")
        
        if results['sharpe_test']['significant']:
            significant_tests.append(f"Sharpe ratio test: {results['sharpe_test']['interpretation']}")
