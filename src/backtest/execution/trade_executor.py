...
class TradeExecutor:
    ...
    def execute_trade(
        ...
        if action == "hold" and quantity > 0:
            if (
                portfolio["stock"] == 0
            ):  # Currently no position, interpret hold+quantity as buy
                action = "buy"
                ...
                if (
                    target_position_value > current_position_value * 1.1
                ):  # Target position is 10% larger than current
                    action = "buy"
                    quantity = (
                        quantity - portfolio["stock"]
                    )  # Calculate additional quantity to buy
                elif (
                    target_position_value < current_position_value * 0.9
                ):  # Target position is 10% smaller than current
                    action = "sell"
                    quantity = (
    ...
    def _execute_buy_order(
        self, quantity: int, price: float, portfolio: dict, date: str, cost_model
    ) -> TradeResult:
        """Execute buy order"""
