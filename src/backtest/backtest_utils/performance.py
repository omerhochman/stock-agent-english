...
class PerformanceAnalyzer:
    ...
    def performance_attribution(
        ...
        attribution = {
            ...
            "beta": (
                np.cov(portfolio_ret, benchmark_ret)[0, 1] / np.var(benchmark_ret)
                if np.var(benchmark_ret) != 0
                else 1
            ),
            "correlation": portfolio_ret.corr(benchmark_ret),
            "up_capture": self._calculate_capture_ratio(
                portfolio_ret, benchmark_ret, "up"
            ),
            "down_capture": self._calculate_capture_ratio(
                portfolio_ret, benchmark_ret, "down"
            ),
        }
    ...
    def _calculate_capture_ratio(
        self, portfolio_returns: pd.Series, benchmark_returns: pd.Series, direction: str
    ) -> float:
